...
// Auto-detect if we are running locally or on Netlify/GitHub Pages
const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.protocol === 'file:';
const API_BASE = isLocal ? 'http://localhost:5000/api' : LIVE_API_BASE;
...
class ApiClient {
    ...
    async _isBackendAvailable() {
        ...
        // If we in production (Netlify/GitHub), we MUST use the live backend.
        // We do not want to fallback to LocalStorage, otherwise devices won't share data.
        if (!isLocal) {
