...
export class AdminDashboard {
                    ...
                    <div class="card-inner">
                        <form id="add-student-form">
                            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                                ...
                                <div><label>Program</label><select name="program" required style="width:100%; padding:8px; border:1px solid #ccc; border-radius:4px;"><option value="UG">UG</option><option value="PG">PG</option></select></div>
    ...
    async downloadExcel() {
        ...
        const groups = {};
        students.forEach(s => {
            const prog = s.program || 'UG';
            const yr = s.year || 1;
            const sheetName = `${prog} Year ${yr}`;
            if (!groups[sheetName]) groups[sheetName] = [];
            groups[sheetName].push({
                "Student ID": s.id || s.register_no || '',
                "Name": s.name || '',
        ...
        for (const sheetName in groups) {
            const ws = XLSX.utils.json_to_sheet(groups[sheetName]);
            XLSX.utils.book_append_sheet(wb, ws, sheetName);
        }

        XLSX.writeFile(wb, "Student_Registry.xlsx");
    }
