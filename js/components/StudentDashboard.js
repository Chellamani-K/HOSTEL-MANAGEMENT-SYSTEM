...
export class StudentDashboard {
    ...
    async render() {
        try {
            ...
            const [student, menu, attendance, leaveRequests, notices] = await Promise.all([
            ...
            if (!student) {
                this.student = { id: this.studentId, name: 'Student', dept: 'N/A', hostel: 'N/A', room: '', year: '-' };
            } else {
            ...
            const safeMenu = menu || { date: new Date().toISOString().split('T')[0], breakfast: 'Not Available', lunch: 'Not Available', dinner: 'Not Available', staff: '-' };
            const safeAttendance = attendance || { breakfast: false, lunch: false, dinner: false, present: true };

            this.container.innerHTML = `
                <div class="dashboard-header">
                    <div>
                        <h2>🎓 Hello, ${this.student.name}</h2>
                        <p>Department: ${this.student.dept} | Hostel: ${this.student.hostel}${this.student.room ? ' | Room: ' + this.student.room : ''} | Year: ${this.student.year}</p>
                    </div>
                ...
                <div class="card-grid">
                        ...
                        <div class="card-inner">
                            ...
                            <div style="display:flex; justify-content:space-between; align-items:center; background: #f5f8fb; padding: 14px; border-radius: 6px;">
                                <div>
                                    ...
                                    <span class="status-badge ${safeAttendance.present ? 'active' : 'inactive'}">
                                        ${safeAttendance.present ? '✔ Present' : '✘ Absent'}
                                    </span>
                                </div>
                                <button class="btn ${safeAttendance.present ? 'btn-danger' : 'btn-primary'}" 
                                    data-action="toggle-presence">
                                    ${safeAttendance.present ? 'Mark Absent' : 'Mark Present'}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-inner">
                            ...
                            ${this.renderMealToggle('Breakfast', safeMenu.breakfast, safeAttendance.breakfast, safeAttendance.present)}
                            ${this.renderMealToggle('Lunch', safeMenu.lunch, safeAttendance.lunch, safeAttendance.present)}
                            ${this.renderMealToggle('Dinner', safeMenu.dinner, safeAttendance.dinner, safeAttendance.present)}
                        </div>
                    </div>
    ...
    attachEvents() {
        // Presence Toggle
        this.container.querySelectorAll('[data-action="toggle-presence"]').forEach(btn => {
            btn.addEventListener('click', async () => {
                try {
                    await db.togglePresence(this.studentId);
                    await this.render();
