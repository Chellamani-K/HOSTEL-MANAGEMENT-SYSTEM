...
class SimulatedDB {
    ...
    init() {
        if (!localStorage.getItem('hfm_students')) {
            localStorage.setItem('hfm_students', JSON.stringify([
                { id: 'S001', name: 'John Doe', dept: 'CS', program: 'UG', year: 2, hostel: 'H1', room: '101', phone: '9876543210' },
                { id: 'S002', name: 'Jane Smith', dept: 'EE', program: 'UG', year: 3, hostel: 'H1', room: '102', phone: '9876543211' },
                { id: 'S003', name: 'Rajesh Kumar', dept: 'ME', program: 'UG', year: 1, hostel: 'H2', room: '201', phone: '9876543212' },
                { id: 'S004', name: 'Priya Devi', dept: 'CS', program: 'PG', year: 2, hostel: 'H1', room: '103', phone: '9876543213' },
                { id: 'S005', name: 'Arun Kumar', dept: 'ECE', program: 'PG', year: 4, hostel: 'H2', room: '202', phone: '9876543214' }
            ]));
        }
    ...
    loginStudent(userId, password) {
        const students = JSON.parse(localStorage.getItem('hfm_students')) || [];
        const student = students.find(s => s.userId === userId && s.password === password || s.user_id === userId && s.password === password);
        if (student) return { message: 'Login successful', student };
        throw new Error('Invalid User ID or Password');
    }

    getStudentAttendanceStatus(studentId, date) {
        const records = JSON.parse(localStorage.getItem('hfm_attendance_status')) || {};
        const key = `${date}_${studentId}`;
        if (records[key]) return { status: records[key] };
        return { status: 'Absent' };
    }

    markStudentAttendance(studentId, status, date) {
        const records = JSON.parse(localStorage.getItem('hfm_attendance_status')) || {};
        const key = `${date}_${studentId}`;
        records[key] = status;
        localStorage.setItem('hfm_attendance_status', JSON.stringify(records));
