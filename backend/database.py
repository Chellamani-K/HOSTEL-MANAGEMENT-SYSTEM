...
def init_db():
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_new_attendance_date ON attendance(date)')
    ...
    if cursor.fetchone()[0] == 0:
        students = [
            ('John Doe', 'REG001', 'CS', 2, '9876543210', 'john@test.com', 'john_cs', 'pass123'),
            ('Jane Smith', 'REG002', 'EE', 3, '9876543211', 'jane@test.com', 'jane_ee', 'pass123'),
            ('Rajesh Kumar', 'REG003', 'ME', 1, '9876543212', 'rajesh@test.com', 'rajesh_me', 'pass123')
        ]
        cursor.executemany('INSERT INTO students (name, register_no, department, year, mobile, email, user_id, password) VALUES (?,?,?,?,?,?,?,?)', students)
    ...
    if cursor.fetchone()[0] == 0:
        ...
        complaints = [
            (1, today, 'Food Quality', 'Sambar was too watery today.', 'Pending', now_iso),
            (2, today, 'Infrastructure', 'Room 102 fan not working properly.', 'In Progress', now_iso),
            (3, today, 'Hygiene / Cleanliness', 'Washroom needs cleaning.', 'Resolved', now_iso),
        ]
        cursor.executemany(
    ...
    if cursor.fetchone()[0] == 0:
        leave_requests = [
            (1, 'John Doe', '2026-03-05', '2026-03-07', 'Family function', 'Pending', today),
            (3, 'Rajesh Kumar', '2026-03-04', '2026-03-04', 'Medical checkup', 'Approved', today),
        ]
        cursor.executemany(
