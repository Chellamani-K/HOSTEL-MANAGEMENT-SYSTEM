...
def add_student():
    """Add a new student (admin panel)"""
    data = request.json
    conn = get_db()
    
    reg_no = data.get('registerNo') or data.get('id', '')
    dept = data.get('department') or data.get('dept', '')
    mobile = data.get('mobile') or data.get('phone', '')
    user_id = data.get('userId') or reg_no
    password = data.get('password') or 'Default@123'
    program = data.get('program', 'UG')
    
    try:
        conn.execute(
            "INSERT INTO students (name, register_no, department, program, year, mobile, email, user_id, password) VALUES (?,?,?,?,?,?,?,?,?)",
            (data['name'], reg_no, dept, program, int(data.get('year', 1)), mobile, data.get('email', ''), user_id, password)
        )
        conn.commit()
...
def register():
    """Register a new student from the portal"""
    data = request.json
    conn = get_db()
    try:
        conn.execute(
            "INSERT INTO students (name, register_no, department, program, year, mobile, email, user_id, password) VALUES (?,?,?,?,?,?,?,?,?)",
            (data['name'], data['registerNo'], data['department'], data.get('program', 'UG'), int(data['year']),
             data['mobile'], data['email'], data['userId'], data['password'])
        )
        conn.commit()
    except Exception as e:
        conn.close()
...
def login():
    ...
    conn = get_db()
    row = conn.execute("SELECT * FROM students WHERE user_id = ? AND password = ?", (user_id, password)).fetchone()
    conn.close()
    if row:
        return jsonify({"message": "Login successful", "student": row_to_dict(row)}), 200
    else:
...
def get_student_attendance_status():
    ...
    conn = get_db()
    row = conn.execute("SELECT * FROM attendance WHERE student_id = ? AND date = ?", (student_id, date_str)).fetchone()
    conn.close()
    if row:
        return jsonify(row_to_dict(row)), 200
    else:
        return jsonify({"status": "Absent"}), 200
